...
(function() {
  ...
  function detectFraming() {
    try {
      ...
      // Redirigir a la página original
      document.body.innerHTML = 'Este sitio no puede ser mostrado en un iframe.';
      setTimeout(function() {
  ...
  function obfuscateContent() {
    ...
    if (window.location.hostname !== 'localhost' && !window.location.hostname.includes('127.0.0.1')) {
      ...
      const elementsToProtect = document.querySelectorAll('p, h1, h2, h3, h4, h5, h6, span, div.content');
      ...
      elementsToProtect.forEach(element => {
        ...
        if (originalContent.length < 10 || element.closest('nav') || element.closest('header') || element.closest('footer')) {
          return;
        }
        ...
        words.forEach((word, index) => {
          ...
          const randomAttr = 'data-' + Math.random().toString(36).substring(2, 8);
  ...
  function detectDevTools() {
    ...
    if (widthThreshold || heightThreshold) {
      devToolsOpen = true;
      console.clear();
      console.log('%c¡Advertencia!', 'color: red; font-size: 30px; font-weight: bold;');
      console.log('%cEste sitio está protegido contra clonación y scraping.', 'font-size: 16px;');
    }
  ...
  function detectBots() {
    ...
    setTimeout(function() {
      ...
      if (mouseMovements < 5 && keyPresses === 0 && scrollEvents < 2) {
        ...
        trapElements.innerHTML = '<a href="/trap-link">Enlace invisible</a><p>Contenido trampa para bots</p>';
  ...
  function initProtection() {
    ...
    console.log('%cAdvertencia de Seguridad', 'color: red; font-size: 20px; font-weight: bold;');
    console.log('%cEste sitio web está protegido contra clonación y scraping.', 'font-size: 14px;');
    console.log('%cCualquier intento de extraer contenido automáticamente viola nuestros términos de servicio.', 'font-size: 14px;');
  }
