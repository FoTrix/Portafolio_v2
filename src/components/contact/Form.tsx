  ...
  return (
    <>
      <form
        ...
        className="w-full max-w-2xl mx-auto bg-white dark:bg-neutral-800/90 p-6 sm:p-8 rounded-xl shadow-2xl border border-neutral-200 dark:border-neutral-700 space-y-6 transition-all duration-300 hover:shadow-blue-500/10"
      >
        <div>
          <label
            htmlFor="email"
        ...
        {status && status !== "Enviando..." && (
          <p
            className={`text-center py-2 font-medium transition-all duration-300 ease-in-out ${status.toLowerCase().includes("error") || status.toLowerCase().includes("fallo") ? "text-red-500 dark:text-red-400" : "text-green-600 dark:text-green-400"}`}
          >
            {status}
          </p>
        )}
